...
Every recipe change is guarded so it never fights another overhaul mod. Everything that adds content is off by default; a few vanilla-behavior restores (crashed-ship pickup, all modules in beacons, quality in asteroid crushing, quality module slots) default on and can be switched off.
...
- **Quality adds module slots to all machines** (on, needs the Quality mod) — every machine with at least one module slot gains extra slots at higher quality, modded machines included; affected machines say so in their tooltip. Replaces the retired QualityEffectsFixed mod.
- **Vanilla engine unit names & icons** (on) — restores the names/icons AAI reskins; turn off if another mod should own their look.
...
Every restore is fingerprint-guarded: a recipe is only touched if it still matches AAI's version or contains an AAI-only item. If another mod already rewrote it, E-Tech leaves it alone. Summary lines are always logged; the "Verbose recipe-restore logging" startup setting adds a per-recipe decision log — search factorio-current.log for [E-Tech].
