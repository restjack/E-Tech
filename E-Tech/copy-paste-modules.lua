-- copy-paste-modules.lua
-- Copies modules along with machine settings on shift-click paste: modules
-- are physically moved from the player's inventory (and the target's old
-- modules handed back), with a bot logistic request created for whatever the
-- player doesn't have. Handles ghosts on both ends and radar/remote view.
-- Vanilla only copies module REQUESTS; this actually moves the items.
--
-- Ported from "Copy Paste Modules" 0.2.x by kajacx (MIT — see
-- LICENSE-third-party.txt). Changes from the original: converted to an
-- event_handler lib, the per-user enable setting renamed etech-cpm-enabled,
-- two unused settings dropped. Gated by the etech-copy-paste-modules startup
-- setting and skipped when the original mod is enabled (control.lua).
-- The furnace/lab/beacon cross-paste registration lives in misc-tweaks.lua.

local function hasItemA(array, pred) -- returns index of found item if yes
...
local function getWhereA(array, pred) -- returns found item or nil
	-- this function works on arrays only (keys: 1-n)
	for i = 1, #array, 1 do
		local item = array[i]
		if pred(item) then
...
local function removeWhereA(array, pred, multiple) -- returns index of item if yes, or the number of removed items if yes and multiple=true
	-- this function works on arrays only (keys: 1-n)
	local removedNum = 0
	local i = 1
	while i <= #array do
...
local function on_entity_settings_pasted(event)
    ...
    if not player or not player.mod_settings["etech-cpm-enabled"].value then
      return -- do nothing
    end
	...
	local function getPlan(name, quality, stackIndex)
		return {
	...
	for i, iteminfo in pairs(modules_to_give) do
		if iteminfo.count > 0 then
			...
			if given < iteminfo.count then -- not all items could be given, "dump" them the ground
				player.print({ "message.etech-cpm-no-inventory-space", prototypes.item[iteminfo.name].localised_name })
		...
		for name, count in pairs(diff) do
			if count ~= 0 then
				player.create_local_flying_text({
					text = {
						(count > 0 and "message.etech-cpm-items-removed" or "message.etech-cpm-items-added"),
						math.abs(count),
