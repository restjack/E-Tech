...
- **Factorissimo map icons** (off, needs the [notnotmelon Factorissimo fork](https://mods.factorio.com/mod/factorissimo-2-notnotmelon)) — mirrors each factory building's first overlay signal (the display-upgrade icons) as a map marker, so your factories are identifiable in map view like machines' recipe icons.
